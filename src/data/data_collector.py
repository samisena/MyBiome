...
class PubMedCollector:
    ...
    def __init__(self):
        ...
        self.data_dir = Path("data/raw/papers")
        self.metadata_dir = Path("data/raw/metadata")
...
"""
Simple test script for PubMedCollector.
This script takes a small sample of probiotic strains and health conditions
and runs the collection pipeline.
"""
import os
import time
import json
...
Path("data/raw/papers").mkdir(parents=True, exist_ok=True)
Path("data/raw/metadata").mkdir(parents=True, exist_ok=True)

# PubMedCollector class with the fixed indentation of run_collection_for_list
class PubMedCollector:
    """The class interacts with PubMed's E-utilities API to:
        1.Search for paper IDs matching specific criteria (strains and conditions)
        2.Fetch paper details in XML format from those IDs
    ...
    def fetch_paper_details(self, pmid_list):
        """Returns the paper details of a paper given its ID"""
        # Error Handling:
        if not pmid_list:   # if pmid_list is empty
            return []   # early return
        pmids = ",".join(pmid_list)   # turns the list into a string separated with commas
        fetch_url = f"{self.base_url}efetch.fcgi"  # fetch endpoint format
        params = {
            "db" : 'pubmed',
            "id" : pmids,
    ...
    def collect_by_strain_and_condition(self, strain, condition, max_results=20):
        """Searches for papers that investigate a probiotic strain and a health condition"""
        # Query combining a probiotic strain and health condition/outcome
        query = f'"{strain}"[Title/Abstract] AND "{condition}"[Title/Abstract] AND "clinical trial"[Publication Type]'
        # Gets the IDs of papers matching the query
        paper_ids = self.search_papers(query, max_results)
    ...
    def run_collection_for_list(self, strains, conditions, results_per_query=10):
        """Run the collection process for multiple strains and conditions"""
        collection_results = []
        for strain in strains:
            for condition in conditions:
                print(f"Collecting papers for {strain} and {condition}...")
        ...
        results_file = self.metadata_dir / "collection_results.json"
        with open(results_file, 'w') as f:
            json.dump(collection_results, f, indent=2) # converts Python to JSON
        return collection_results
