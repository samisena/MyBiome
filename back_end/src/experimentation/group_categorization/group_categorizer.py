...
No explanations. Just the JSON array."""
    ...
    def propagate_to_interventions(self) -> Dict[str, int]:
        """
        Propagate categories from groups to interventions (Option A).
        ...
        # Update interventions with categories from their canonical groups
        update_query = """
        UPDATE interventions
        SET intervention_category = (
            SELECT cg.layer_0_category
            FROM canonical_groups cg
            JOIN semantic_hierarchy sh
                ON cg.canonical_name = sh.layer_1_canonical
        ...
        logger.info(f"Propagated categories to {updated_count} interventions, {orphan_count} orphans remaining")
