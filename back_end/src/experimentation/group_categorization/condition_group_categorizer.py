...
No explanations. Just the JSON array."""
    ...
    def propagate_to_conditions(self) -> Dict:
        ...
        Uses UPDATE-JOIN to set condition_category based on group membership.
        ...
        # Update interventions.condition_category from canonical_groups via semantic_hierarchy
        update_query = """
        ...
        SET condition_category = (
            ...
            WHERE sh.entity_name = interventions.health_condition
            AND sh.entity_type = 'condition'
            AND cg.entity_type = 'condition'
            AND cg.layer_0_category IS NOT NULL
            LIMIT 1
        )
        WHERE health_condition IN (
            SELECT entity_name
            FROM semantic_hierarchy
            WHERE entity_type = 'condition'
        )
        ...
        logger.info(f"Updated {updated_count} condition assignments")
