...
class DatabaseMigration:
    ...
    def _backup_database(self) -> Optional[Path]:
        ...
        # Skip backup creation in FAST_MODE (performance optimization)
        if config.fast_mode:
            return None
        ...
        # Database backup creation (disabled in FAST_MODE)
            ...
            try:
                if self.current_version == 1 and self.target_version >= 2:
                    ...
                    self._record_migration(
                        ...
                        notes=f"Backup created at: {backup_path}" if backup_path else "No backup created (FAST_MODE)"
                    )
                ...
                if backup_path:
                    ...
                    logger.error("No backup available (FAST_MODE was enabled)")
                raise
