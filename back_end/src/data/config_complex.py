...
@dataclass
class LLMConfig:
    """Individual LLM configuration settings."""
...
class MultiLLMConfig:
    """Configuration for multiple LLMs in consensus-based extraction."""
    models: List[LLMConfig] = None
    consensus_threshold: float = 1.0  # Require all models to agree (100% consensus)
    conflict_resolution: str = "manual_review"  # "manual_review" or "third_model"
    
    def __post_init__(self):
        if self.models is None:
            # Default to gemma2:9b and qwen2.5:14b
            self.models = [
                LLMConfig(
...
class Config:
    ...
    def __init__(self):
        ...
        self.llm = LLMConfig()  # Keep for backward compatibility
        self.multi_llm = MultiLLMConfig()  # New multi-LLM configuration
