...
class SemanticMerger:
    ...
    def __init__(self, primary_model: str = "qwen2.5:14b",
        ...
        # SemanticMerger initialized (logging removed for performance)
    ...
    def _conditions_are_related(self, cond1: str, cond2: str) -> bool:
        """Check if two conditions are related. IBS subtypes are kept separate."""
        # Define condition families - DO NOT group IBS subtypes together
        # IBS-D, IBS-C, IBS-M are distinct conditions with different treatments
        general_ibs = ['ibs', 'irritable bowel syndrome']  # Only general IBS terms
        gerd_variants = ['gerd', 'reflux', 'gastroesophageal', 'gastro-esophageal']

        # Check if both conditions belong to the same family
        for family in [general_ibs, gerd_variants]:
