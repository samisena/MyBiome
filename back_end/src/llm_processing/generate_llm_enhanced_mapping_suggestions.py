...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate LLM-enhanced mapping suggestions")
    parser.add_argument('--quiet', '-q', action='store_true',
                       help='Run in quiet mode with minimal output')
    parser.add_argument('--database', '-d', default="data/processed/intervention_research.db",
                       help='Path to the database file')
    parser.add_argument('--batch-size', '-b', type=int, default=20,
                       help='Batch size for LLM processing (default: 20)')
    args = parser.parse_args()

    # Override FAST_MODE if quiet flag is set
    global FAST_MODE
    if args.quiet:
        FAST_MODE = True
    ...
    if not os.path.exists(db_path):
        print(f"Error: Database not found at {db_path}")  # Always show errors
        return
    ...
    for entity_type in ['condition', 'intervention']:
        ...
        if unmapped_terms:
            ...
            suggestions = generate_llm_enhanced_suggestions(normalizer, unmapped_terms, entity_type, batch_size=args.batch_size)
...
if __name__ == "__main__":
    try:
        ...
        print("\n[INTERRUPTED] Process stopped by user")  # Always show interruption
