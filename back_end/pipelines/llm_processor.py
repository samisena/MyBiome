...
class ThermalMonitor:
    ...
    def __init__(self, max_temp: float = 80.0, cooling_temp: float = 70.0,
        ...
        logger.info(f"Thermal monitor initialized - Max: {max_temp}°C, Cool: {cooling_temp}°C")
    ...
    def is_thermal_safe(self) -> Tuple[bool, Optional[ThermalStatus]]:
        ...
        if not status:
            logger.warning("Cannot read thermal status, assuming safe")
            return True, None
        ...
        if len(self.temp_history) >= 3:
            ...
            if len(recent_temps) >= 2:
                ...
                if temp_rate > 2 and status.gpu_temp > (self.max_temp - 5):
                    logger.warning(f"GPU heating rapidly: {temp_rate:.1f}°C/min, current: {status.gpu_temp}°C")
    ...
    def wait_for_cooling(self) -> None:
        ...
        while True:
            ...
            if status.gpu_temp <= self.cooling_temp:
                ...
                logger.info(f"🌡️ System cooled to {status.gpu_temp}°C after {cooling_duration/60:.1f} minutes")
                break

            logger.info(f"🌡️ Cooling... GPU: {status.gpu_temp}°C, CPU: {status.cpu_temp}°C, Target: {self.cooling_temp}°C")
            time.sleep(30)
    ...
    def _monitor_loop(self):
        ...
        while self.is_monitoring:
            ...
            if status and not status.is_safe:
                ...
                logger.warning(f"Thermal warning: GPU {status.gpu_temp}°C, CPU {status.cpu_temp}°C")
...
class MemoryManager:
    ...
    def cleanup_memory(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Memory cleanup performed (#{self.cleanup_count})")
            return True
        except Exception as e:
...
class LLMProcessor:
        ...
        try:
            ...
            for i, paper in enumerate(papers_to_process):
                ...
                try:
                    ...
                    if not is_safe:
                        logger.warning(f"🌡️ Thermal protection triggered")
    ...
    def _generate_final_report(self, session: ProcessingSession) -> Dict[str, Any]:
        ...
        logger.info(f"🔥 Thermal events: {len(session.progress.thermal_events)}")
